...
- `a_query` and `b_query`: The queries you want to compare.
- `primary_key`: The primary key of the model. Used to sort unmatched results for row-by-row validation. Must be a unique key (unique and never `null`) in both tables, otherwise the join won't work as expected.
...
You may want to print the output of the query generated by an audit helper macro to your logs (instead of previewing the results).
